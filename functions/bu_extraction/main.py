...
def get_business_unit(api_endpoint, client_id, client_secret, st_app_key, region_name, tenant_id, desired_timezone_name):
  df = pd.DataFrame()
  ...
  try:
      ...
      if len(data) > 0:
        ...
        data.drop(['externalData'], axis=1, inplace=True)
        new_df = data.copy()
        ...
        dates_to_fix = {
            'business-units': ['createdOn', 'modifiedOn'],
        }
...
def extract_business_units(event, context):
  try:
    ...
    print(json.dumps({"status": {"code": 400, "text": f"Error decoding Pub/Sub message: {e}"}}))
    raise
  ...
  try:
    ...
    organizations = read_big_query_table(credentials_path='cred.json', project_id=project_id, dataset_id=dataset_id, table_id='organization_information')
  ...
  api_endpoint = f'https://api.servicetitan.io/settings/v2/tenant/{tenant_id}/business-units'
        
  try:
    bu_df = get_business_unit(api_endpoint, client_id, client_secret, st_app_key, region_name, tenant_id, timezone_zone) 
  except Exception as e:
    print(json.dumps({'status':{'code': 400, 'text': f'Error while extracting data from Service Titan for Tenant Id {tenant_id}: {e}'}}))
    raise

  try:
    load_status = bigquery_insert(bu_df, tenant_id, project_id=project_id, dataset_id=dataset_id, table_id='st_get_raw_settings_business_unit')  
