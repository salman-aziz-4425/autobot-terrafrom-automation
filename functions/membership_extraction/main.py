...
def get_members(api_endpoint,client_id,client_secret,st_app_key,region_name,tenant_id,desired_timezone_name):
  df = pd.DataFrame()
  ...
  params = {
        ...
        , "pageSize": '5000'
        ,'includeTotal':'true'
        ,'modifiedOnOrAfter': str(datetime.now().date() - timedelta(days=7))
  }
  ...
  try:
      ...
      if response.status_code == 200:
          # Check if data is empty and totalCount is 0
          if not data and total_count == 0:
              print("No modified data available.")
              return pd.DataFrame()
      ...
      if len(data) > 0:
        ...
        dates_to_fix = {
            'memberships': ['createdOn', 'modifiedOn', 'followUpOn', 'cancellationDate', 'from', 'nextScheduledBillDate', 'to'],
        }
...
def extract_memberships(event, context):
  ...
  api_endpoint = f'https://api.servicetitan.io/memberships/v2/tenant/{tenant_id}/memberships'

  try:
    members_df = get_members(api_endpoint, client_id, client_secret, st_app_key, region_name, tenant_id, timezone_zone)
    if members_df.empty:
      print("No modified data returned from API.")
      return
  except Exception as e:
  ...
  try:
    print(f'----------------------------------- Deleting Data from {str(datetime.now().date() - timedelta(days=7))} -----------------------------------')
    delete_big_query_data(credentials_path='cred.json', project_id = project_id, dataset_id = dataset_id, table_id = 'st_get_raw_membership_memberships',
    target_column = 'modifiedOn', target_value = str(datetime.now().date() - timedelta(days=7)))
  except Exception as e:
    print(json.dumps({'status':{'code': 400, 'text': f'Error while deleting data from the table: {e}'}}))
    raise

  try:
    load_status = bigquery_insert(members_df, tenant_id, project_id=project_id, dataset_id=dataset_id, table_id='st_get_raw_membership_memberships', write_disposition='WRITE_APPEND')
    print(json.dumps({'status': {'code': 200, 'text':'Success'}}))
  except:
    load_status = 'Error while dumping data into Big Query for Customers.'
