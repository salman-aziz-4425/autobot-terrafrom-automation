...
def get_membership_types(api_endpoint,client_id,client_secret,st_app_key,region_name,tenant_id,desired_timezone_name):
  df = pd.DataFrame()
  ...
  try:
      ...
      if len(data) > 0:
        ...
        dates_to_fix = {
            'membership-types': ['createdOn', 'modifiedOn'],
        }
...
def extract_membership_types(event, context):
  ...
  api_endpoint = f'https://api.servicetitan.io/memberships/v2/tenant/{tenant_id}/membership-types'

  try:
    membership_types_df = get_membership_types(api_endpoint, client_id, client_secret, st_app_key, region_name, tenant_id, timezone_zone)
  except Exception as e:
    print(json.dumps({'status':{'code': 400, 'text': f'Error while extracting data from Service Titan for Tenant Id {tenant_id}: {e}'}}))
    raise

  try:
    load_status = bigquery_insert(membership_types_df, tenant_id, project_id=project_id, dataset_id=dataset_id, table_id='st_get_raw_membership_membership_types')
