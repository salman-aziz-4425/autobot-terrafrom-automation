...
def get_customer_contact(api_endpoint,client_id,client_secret,st_app_key,region_name,tenant_id,desired_timezone_name):
  df = pd.DataFrame()
...
def extract_cdc_customer_contact(event, context):
    ...
    api_endpoint = f'https://api.servicetitan.io/crm/v2/tenant/{tenant_id}/customers/contacts'

    try:
      customer_contact_df = get_customer_contact(api_endpoint, client_id, client_secret, st_app_key, region_name, tenant_id, timezone_zone)        
    except Exception as e:
    ...
    try:
      ...
      delete_big_query_data(credentials_path='cred.json', project_id = project_id, dataset_id = dataset_id, table_id = 'st_get_raw_crm_customers_contacts',
      target_column = 'modifiedOn', target_value = str(datetime.now().date() - timedelta(days=121)))
    except Exception as e:
      print(json.dumps({'status':{'code': 400, 'text': f'Error while deleting data from BQ: {e}'}}))
      raise

    try:
      print(f'----------------------------------- Loading Data in Big Query -----------------------------------')
      load_status = bigquery_insert(customer_contact_df, tenant_id, project_id=project_id, dataset_id=dataset_id, table_id='st_get_raw_crm_customers_contacts', write_disposition='WRITE_APPEND')  
