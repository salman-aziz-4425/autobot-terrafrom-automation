...
def get_customers(api_endpoint, client_id, client_secret, st_app_key, region_name, tenant_id, desired_timezone_name):
  df = pd.DataFrame()
  ...
  params = {
        ...
        # ,'createdOnOrAfter': str(datetime.now().date() - timedelta(days=7))
        # ,'createdBefore': str(datetime.now().date())
  }
  ...
  try:
      ...
      if len(data) > 0:
        ...
        dates_to_fix = {
            'customers': ['createdOn', 'modifiedOn'],
        }
...
def extract_customers(event, context):
  ...
  api_endpoint = f'https://api.servicetitan.io/crm/v2/tenant/{tenant_id}/customers'

  try:
    customer_df = get_customers(api_endpoint, client_id, client_secret, st_app_key, region_name, tenant_id, timezone_zone)
  except Exception as e:
  ...
  try:
    ...
    delete_big_query_data(credentials_path='cred.json', project_id = project_id, dataset_id = dataset_id, table_id = 'st_get_raw_crm_customers',
  ...
  try:
    load_status = bigquery_insert(customer_df, tenant_id, project_id=project_id, dataset_id=dataset_id, table_id='st_get_raw_crm_customers', write_disposition='WRITE_APPEND')
