...
def get_technicians(api_endpoint, client_id, client_secret, st_app_key, region_name, tenant_id, desired_timezone_name):
  df = pd.DataFrame()
  ...
  try:
      ...
      if len(data) > 0:
        ...
        dates_to_fix = {
            'technicians': ['createdOn', 'modifiedOn'],
        }
...
def extract_technician(event, context):
  ...
  api_endpoint = f'https://api.servicetitan.io/settings/v2/tenant/{tenant_id}/technicians'
        
  try:
    tec_df = get_technicians(api_endpoint, client_id, client_secret, st_app_key, region_name, tenant_id, timezone_zone) 
  except Exception as e:
    print(json.dumps({'status':{'code': 400, 'text': f'Error while extracting data from Service Titan for Tenant Id {tenant_id}: {e}'}}))
    raise

  try:
    load_status = bigquery_insert(tec_df, tenant_id, project_id=project_id, dataset_id=dataset_id, table_id='st_get_raw_settings_technicians')  
