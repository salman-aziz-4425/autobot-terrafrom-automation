...
def get_campaigns(api_endpoint, client_id, client_secret, st_app_key, region_name, tenant_id, desired_timezone_name):
  df = pd.DataFrame()
  ...
  try:
      ...
      if len(data) > 0:
        ...
        columns_to_flatten = {
          'campaigns': ['campaignPhoneNumbers'],
        }
        ...
        dates_to_fix = {
            'campaigns': ['modifiedOn', 'createdOn'],
        }
...
def extract_campaigns(event, context):
  # Decode the Pub/Sub message
    ...
    api_endpoint = f'https://api.servicetitan.io/marketing/v2/tenant/{tenant_id}/campaigns'

    try:
      campaigns_df = get_campaigns(api_endpoint, client_id, client_secret, st_app_key, region_name, tenant_id, timezone_zone)  
    except Exception as e:
      print(json.dumps({'status':{'code': 400, 'text': f'Error while extracting data from Service Titan for Tenant Id {tenant_id}: {e}'}}))
      raise

    try:
      load_status = bigquery_insert(campaigns_df, tenant_id, project_id=project_id, dataset_id=dataset_id, table_id='st_get_raw_marketing_campaigns')  
