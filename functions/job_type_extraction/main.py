...
def get_job_type(api_endpoint, client_id, client_secret, st_app_key, region_name, tenant_id, desired_timezone_name):
  df = pd.DataFrame()
  ...
  try:
      ...
      if len(data) > 0:
        ...
        data.drop(['tagTypeIds', 'externalData'], axis=1, inplace=True)
        new_df = data.copy()

        columns_to_flatten = {
          'job-types': ['businessUnitIds'],
        }
...
def extract_job_types(event, context):
    ...
    api_endpoint = f'https://api.servicetitan.io/jpm/v2/tenant/{tenant_id}/job-types'

    try:
      job_type_df = get_job_type(api_endpoint, client_id, client_secret, st_app_key, region_name, tenant_id, timezone_zone)        
    except Exception as e:
      print(json.dumps({'status':{'code': 400, 'text': f'Error while extracting data from Service Titan for Tenant Id {tenant_id}: {e}'}}))    
      raise
    
    try:
      load_status = bigquery_insert(job_type_df, tenant_id, project_id=project_id, dataset_id=dataset_id, table_id='st_get_raw_jpm_job_type')  
